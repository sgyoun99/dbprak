...
CREATE TABLE item(item_id text PRIMARY KEY, title TEXT, rating numeric(2,1), salesranking INTEGER, image TEXT, productgroup int NOT NULL);
CREATE TABLE shop(shop_id SERIAL, shop_name TEXT NOT NULL, street TEXT NOT NULL, zip CHAR(5) NOT NULL, PRIMARY KEY(shop_id), UNIQUE(shop_name, street, zip));
CREATE TABLE item_shop(item_id text NOT NULL, shop_id Integer NOT NULL, currency CHAR(3), price numeric(8,2), availabiliti BOOLEAN, condition text NOT NULL, PRIMARY KEY(item_id, shop_id, condition), FOREIGN KEY(item_id) REFERENCES item(item_id) ON DELETE CASCADE, FOREIGN KEY(shop_id) REFERENCES shop(shop_id) ON DELETE CASCADE);
CREATE TABLE similar_items(item_id text, similar_item_id text, PRIMARY KEY(item_id, similar_item_id), FOREIGN KEY(item_id) REFERENCES item(item_id) ON DELETE CASCADE, FOREIGN KEY (similar_item_id) REFERENCES item(item_id) ON DELETE CASCADE);
...
CREATE TABLE dvd(item_id text PRIMARY KEY, format TEXT, runningtime SMALLINT, regioncode CHAR(5), FOREIGN KEY (item_id) REFERENCES item(item_id) ON DELETE CASCADE);
...
CREATE TABLE customer(name TEXT NOT NULL UNIQUE, street text NOT NULL, nr SMALLINT NOT NULL, zip SMALLINT NOT NULL, city TEXT NOT NULL, account_number TEXT NOT NULL UNIQUE);
...
*/CREATE TABLE review(review_id SERIAL PRIMARY KEY, item_id TEXT NOT NULL, customer text NOT NULL, review_date DATE, summary TEXT, content TEXT, rating SMALLINT, FOREIGN KEY (item_id) REFERENCES item(item_id) ON DELETE CASCADE, FOREIGN KEY (customer) REFERENCES customer(name) ON DELETE CASCADE);
